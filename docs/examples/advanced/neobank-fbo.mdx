...
**Normal credit accounts** — these represent your liabilities to customers. Each `$customer_id` has an `available` balance (spendable funds), a `holds` subtree (one account per card authorization, by `$auth_id`), a `withdrawals` subtree (one `pending` account per outbound ACH, by `$withdrawal_id`), and an `advances` subtree (one `outstanding` receivable per advance, by `$advance_id`). The advance `outstanding` accounts run negative while open — they are receivables the platform fronts — and drain to zero when the expected deposit settles.
...
`CARD_AUTH` places a hold on the customer's available balance, moving funds into a per-authorization `holds:$auth_id` account.
...
`ACH_WITHDRAWAL_RESERVE` earmarks funds from the customer's available balance into a per-withdrawal `withdrawals:$withdrawal_id:pending` account.
...
Early-access advances let the platform front money against an expected deposit, booked as a receivable on a per-advance `advances:$advance_id:outstanding` account.
